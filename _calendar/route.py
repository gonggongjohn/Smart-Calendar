...
def add_schedule():
    ...
    try:
        ...
        if session.get('user') is not None:
            ...
            schedule = Schedule(user=user, name=name, category=category, start=start_datetime, end=end_datetime)
...
def fetch_schedule():
    if session.get('user') is not None:
        ...
        for schedule in schedule_item:
            category = db.session.query(Schedule).filter_by(id=schedule.category).first()
            schedule_list.append({'name': schedule.name, 'category': {'id': category.id, 'name': category.name},
                                  'start': schedule.start.timestamp(), 'end': schedule.end.timestamp()})
        status = 1
        if status == 1:
            return json.dumps({'status': status, 'schedule': schedule_list})
        else:
            return json.dumps({'status': status})
    else:
...
def get_category():
    ...
    return json.dumps({'status': 1, 'category': category_list})
